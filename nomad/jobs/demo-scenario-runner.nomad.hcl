job "demo-scenario-runner" {
  datacenters = ["dc1"]
  type = "service"
  group "app" {
    task "demo-scenario-runner" {
      driver = "docker"
      config { image = "${NOMAD_ENTERPRISE_IMAGE_URI}" command = "sh" args = ["-c", "echo demo-scenario-runner placeholder && sleep 3600"] }
