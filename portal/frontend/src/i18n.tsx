...
const KOREAN_COPY: Record<string, string> = {
  ...
  "Evidence mode": "근거 모드",
  "Shared Ollama": "공유 Ollama",
  "Shared Ollama · {model}": "공유 Ollama · {model}",
  "Amazon Bedrock · {model}": "Amazon Bedrock · {model}",
  ...
  "Secret exposure detected": "시크릿 노출 감지",
  "Dynamic DB credential issued": "동적 DB 자격 증명 발급",
  "PostgreSQL pgAudit event": "PostgreSQL pgAudit 이벤트",
  ...
  Cases: "케이스",
  "Ownership, evidence, comments, and SLA tracking for active investigations":
    "진행 중인 조사의 담당자, 근거, 댓글 및 SLA 추적",
  "No portal role": "포털 역할 없음",
  "Authenticated user": "인증된 사용자",
  "Read-only session": "읽기 전용 세션",
  Authenticated: "인증됨",
  "Authentication pending": "인증 대기",
  "The requested operation failed": "요청한 작업에 실패했습니다.",
  "Direct Vault connection": "Vault 직접 연결",
  "Read-only health, PKI, and lease metadata from the Vault API":
    "Vault API의 읽기 전용 상태, PKI 및 lease 메타데이터",
  "Not observed": "관측되지 않음",
  "Vault cluster": "Vault 클러스터",
  Unsealed: "봉인 해제",
  "Version unavailable": "버전 정보 없음",
  "PKI inventory": "PKI 인벤토리",
  "{count} certificates": "인증서 {count}개",
  "{count} issuers · {roles} roles": "발급자 {count}개 · 역할 {roles}개",
  "Dynamic leases": "동적 lease",
  "Read-only lease metadata": "읽기 전용 lease 메타데이터",
  "Lease prefix not configured": "Lease 접두사 미구성",
  "Read-only identity": "읽기 전용 ID",
  "{minutes} min TTL": "TTL {minutes}분",
  "{count} policies · {mounts} mounts": "정책 {count}개 · 마운트 {mounts}개",
  "Source: Vault API · mode: read-only · observed {time}":
    "소스: Vault API · 모드: 읽기 전용 · 관측 {time}",
  "Investigation case queue": "조사 케이스 큐",
  "Track ownership, evidence, comments, and response SLA":
    "담당자, 근거, 댓글 및 대응 SLA를 추적합니다",
  "New case": "새 케이스",
  "Sign in as a security analyst to create or update cases.":
    "케이스를 생성하거나 변경하려면 보안 분석가로 로그인하세요.",
  Title: "제목",
  Description: "설명",
  Cancel: "취소",
  "Creating...": "생성 중...",
  ...
  "No investigation cases": "조사 케이스 없음",
  "Create a case from the selected finding when ownership or follow-up is required.":
    "담당자 지정이나 후속 조치가 필요하면 선택한 탐지 결과에서 케이스를 생성하세요.",
  "Updated {time}": "업데이트 {time}",
  "SLA due": "SLA 기한",
  "Source reference": "소스 참조",
  Manual: "수동",
  Evidence: "근거",
  "Start investigation": "조사 시작",
  Resolve: "해결",
  "Attach selected finding": "선택 탐지 결과 첨부",
  "No evidence attached yet.": "아직 첨부된 근거가 없습니다.",
  Comments: "댓글",
  "Add comment": "댓글 추가",
  "Add a concise investigation note": "간결한 조사 메모 추가",
  "Post comment": "댓글 등록",
  "Select a case": "케이스 선택",
  "Choose a case to review ownership, evidence, and response progress.":
    "담당자, 근거 및 대응 진행 상황을 검토할 케이스를 선택하세요.",
  "Two-person approval queue": "2인 승인 큐",
  "Requests expire automatically and cannot be approved twice by the same identity":
    "요청은 자동 만료되며 동일 사용자가 두 번 승인할 수 없습니다",
  "{count} pending": "대기 {count}건",
  "Vault Radar rescan": "Vault Radar 재스캔",
  "Re-scan the approved sources after analyst review":
    "분석가 검토 후 승인된 소스를 다시 스캔합니다",
  "Vault PKI renewal": "Vault PKI 갱신",
  "Prepare certificate renewal after two independent approvals":
    "서로 다른 2명의 승인 후 인증서 갱신을 준비합니다",
  "Vault lease revocation": "Vault lease 폐기",
  "Prepare lease revocation after evidence and ownership review":
    "근거와 담당자 검토 후 lease 폐기를 준비합니다",
  "Request approval": "승인 요청",
  "A permitted Keycloak role is required to create or approve requests.":
    "요청 생성 또는 승인에는 허용된 Keycloak 역할이 필요합니다.",
  "Approval progress": "승인 진행",
  "{count}/2 approvals": "승인 {count}/2",
  "Expires {time}": "만료 {time}",
  Approve: "승인",
  Reject: "거부",
  "A different reviewer must provide the second approval.":
    "두 번째 승인은 다른 검토자가 해야 합니다.",
  Dispatch: "실행 전달",
  "Execution disabled": "실행 비활성",
  "Execution remains disabled by policy.": "정책에 따라 실행이 비활성 상태입니다.",
  "No approval requests": "승인 요청 없음",
  "Create a reviewed request above; execution remains disabled until policy explicitly enables it.":
    "위에서 검토 요청을 생성하세요. 정책이 명시적으로 허용할 때까지 실행은 비활성입니다.",
  "Data trust and freshness": "데이터 신뢰도 및 최신성",
  "Observed time, freshness threshold, provenance, and fallback state by source":
    "소스별 관측 시각, 최신성 임계값, 출처 및 fallback 상태",
  "Generated {time}": "생성 {time}",
  "Awaiting live assessment": "라이브 평가 대기",
  "No live timestamp": "라이브 시각 없음",
  "{duration} old": "{duration} 경과",
  "Threshold {duration}": "임계값 {duration}",
  "Freshness assessment unavailable": "최신성 평가 사용 불가",
  "The portal will label live, stale, and fallback sources after the freshness API responds.":
    "최신성 API가 응답하면 live, stale 및 fallback 소스를 표시합니다.",
};
...
const KOREAN_LABELS: Record<string, string> = {
  ...
  met: "준수",
  "not set": "미설정",
  "pending first approval": "1차 승인 대기",
  "pending second approval": "2차 승인 대기",
  approved: "승인됨",
  dispatched: "전달됨",
  rejected: "거부됨",
  expired: "만료됨",
  "cert renew": "인증서 갱신",
  "lease revoke": "lease 폐기",
  rescan: "재스캔",
};
